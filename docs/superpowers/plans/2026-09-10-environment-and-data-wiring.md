...
**Files:**
- Modify: `pyproject.toml` (add missing runtime deps + a dev dependency group)
...
- Create (generated by Step 3 below, then committed): `renv.lock`, `.Rprofile`, `renv/activate.R`, `renv/settings.json`, `renv/.gitignore`
...
- [ ] **Step 1: Fix pyproject.toml — it's missing real runtime and dev dependencies**

Verified by baseline-testing a fresh `uv sync` in a clean worktree:
`matplotlib` and `seaborn` are imported by `2_global_network_metrics/global_basic_metrics.py`
and several `3_nodal_network_metrics/*.py` scripts, but are not declared
anywhere in `pyproject.toml` — the old `install.sh` only ever installed
them via a hardcoded `uv pip install ... matplotlib seaborn` line, which
Step 6 below removes in favor of `uv sync`. Without this fix, `uv sync`
would stop installing them and those scripts would break. Likewise
`pytest`/`pytest-cov` (used by the entire test suite and referenced by
`0_installation/README.md`'s own verification instructions) are not
declared anywhere, so a fresh `uv sync` cannot run the tests at all.

In `pyproject.toml`, add `"matplotlib>=3.5.0"` and `"seaborn>=0.11.0"` to
both the `dependencies` list and the `default` optional-dependency list
(matching how the existing `default` list already duplicates
...
# Temporal analysis pipeline dependencies
temporal = [
    "umap-learn>=0.5.0",
    "scikit-learn>=1.0.0",
]
...
And add a `dev` dependency group (installed by `uv sync` automatically —
uv includes the `dev` group by default unless `--no-dev` is passed), right
...
Verify: `uv sync --all-extras` (from repo root, or inside this worktree),
then `uv run pytest --version` should print a version instead of
`ModuleNotFoundError`.

- [ ] **Step 2: Write the one-time lockfile-generation script**
...
- [ ] **Step 3: Run it once from the repo root to generate the lockfile**
...
- [ ] **Step 4: Write the every-install restore script**
...
- [ ] **Step 5: Verify the restore script works against the freshly generated lock**
...
- [ ] **Step 6: Verify all required packages actually load**
...
- [ ] **Step 7: Rewrite install.sh's dependency-install and lock-file steps**
...
- [ ] **Step 8: Update the "Finalize" step's printed instructions**
...
- [ ] **Step 9: Run install.sh end-to-end to confirm it works unattended**
...
- [ ] **Step 10: Update 0_installation/README.md's R section**
...
git add pyproject.toml uv.lock \
        0_installation/init_r_env.R 0_installation/setup_r_env.R \
...
pyproject.toml gained matplotlib/seaborn (imported by stages 2-3 but
previously only installed via install.sh's hardcoded pip list) and a
pytest/pytest-cov dev dependency group (previously not declared
anywhere, despite the whole test suite depending on them). renv.lock
pins mgcv/lme4/lmerTest/emmeans/tidyverse/jsonlite/argparse/R.matlab/
arrow, scoped to this repo's renv/library/.
